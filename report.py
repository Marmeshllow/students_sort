...
def make_report_about_top3(students_avg_scores):
    res = sorted(students_avg_scores.items(), key=lambda x: x[1], reverse=True)
...
def make_report_about_top3_v2(students_avg_scores):
    res = sorted(students_avg_scores.items(), key=lambda x: x[1], reverse=True)
...
def make_report_about_top3_v3(students_avg_scores):
    res = sorted(students_avg_scores.items(), key=lambda x: x[1], reverse=True)
